...
const calculateAchievements = (group: Group): Achievement[] => {
  ...
  const totalCompletions = group.habits.reduce((acc, habit) => 
    acc + (habit.completions?.length || 0), 0);
        ...
        {achievements.map(achievement => (
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className={`font-bold ${achievement.unlocked ? 'text-white dark:text-white' : 'text-gray-900 dark:text-white'}`}>
                  {achievement.title}
                </h3>
                <p className={`text-sm ${achievement.unlocked ? 'text-white/90 dark:text-white/90' : 'text-gray-600 dark:text-gray-400'}`}>
            ...
            <div className="space-y-2">
              <div className={`flex justify-between text-sm ${achievement.unlocked ? 'text-white/90 dark:text-white/90' : 'text-gray-600 dark:text-gray-400'}`}>
                <span>Progress</span>
                <span>{achievement.progress}/{achievement.target}</span>
              </div>
              <div className={`h-2 rounded-full ${achievement.unlocked ? 'bg-white/20 dark:bg-white/10' : 'bg-gray-200 dark:bg-gray-700'} overflow-hidden`}>
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${(achievement.progress / achievement.target) * 100}%` }}
                  transition={{ duration: 1, ease: "easeOut" }}
                  className={`h-full rounded-full ${achievement.unlocked ? 'bg-white' : `bg-gradient-to-r ${categoryColors[achievement.category]}`}`}
                />
              </div>
            </div>
