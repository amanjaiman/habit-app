...
export default function DayView({ date, habits }: DayViewProps) {
  ...
  const formatCategory = useCallback(
    (categoryId: string, habit: Habit) => {
      const category = DEFAULT_CATEGORIES.find(
        (c: HabitCategory) => c.id === categoryId
      );
      return (
        <div className="flex items-center gap-2">
          <span
            ...
            style={{
              color: theme === "dark" ? category?.darkColor : category?.color,
            }}
          >
            {category?.name}
          </span>
  ...
  const calculateGroupHabitStreak = (
    ...
    while (true) {
      ...
      if (
        dateStr !== todayStr &&
        (!completion || !isFullyCompleted(habit, completion.completed))
      )
        break;
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          {habit.category && (
                            <div>{formatCategory(habit.category, habit)}</div>
                          )}
                        </div>
                      </div>
                      <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {getAllGroupHabits().map((habit) => {
                ...
                const isCompleted =
                  ...
                  isFullyCompleted(habit, completionValue);
                  ...
                  >
                    <div className="p-3">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() =>
                            handleGroupHabitClick(habit.groupId, habit)
                          }
